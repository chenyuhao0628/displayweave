...
export const releaseTag = "v0.2.1-p6"
export const releaseBase = `https://downloads.urlget.cyou/releases/${releaseTag}`
...
export const copy: Record<Locale, SiteCopy> = {
  en: {
    ...
    release: { label: `${releaseTag} release`, title: "Install once, then update Mac and Android in place.", intro: "Release packages are mirrored as immutable static files through Cloudflare. Android p6 uses Cloudflare first and falls back to the matching GitHub Release when the mirror is unavailable.", warning: "Development distribution — no App Store, Google Play, Developer ID notarization, or universal iOS signing.", checksum: "Verify all downloads with SHA256SUMS.txt", assets: enAssets },
  ...
  zh: {
    ...
    hero: { titleLines: ["把每一块闲置屏幕，", "编入你的"], accent: "Mac 工作空间。", body: "DisplayWeave 将 iPhone、iPad 和 Android 设备变成本地扩展显示器，支持 Apple USB/WiFi、Android ADB USB、已验证的 Mac/Android 更新渠道、触摸、HEVC 与实验性高刷新。", primary: `获取 ${releaseTag}`, secondary: "查看源代码", live: "实时传输拓扑" },
    topology: { mac: "Mac 发送端", source: "采集 · 编码", android: "Android 接收端", apple: "Apple 接收端", usb: "ADB USB", wifi: "本地 WiFi", fallback: "自动回退", active: "已连接" },
    release: { label: `${releaseTag} 发布`, title: "首次安装，之后原位更新 Mac 与 Android。", intro: "发布文件通过 Cloudflare 不可变静态镜像分发。Android p6 优先使用 Cloudflare，镜像不可用时回退到同一 GitHub Release。", warning: "开发分发版——未通过 App Store、Google Play、Developer ID 公证或通用 iOS 签名分发。", checksum: "使用 SHA256SUMS.txt 验证全部下载", assets: zhAssets },
