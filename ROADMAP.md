...
- **下一步：** 长时间稳定性、异常恢复、基准导出、多设备测试矩阵和
  正式发布准备。
- **未来能力：** Android USB/ADB reverse、加密 WiFi 配对、iOS/iPadOS
  120Hz；这些目前都不是已支持功能。
...
- Android USB transport using ADB reverse behind the existing transport
  abstraction. This is planned and is not currently supported.
