...
Auto evaluates receiver statistics only. Congestion reduces the target by 20% after the one-second decrease hold. Five stable seconds permit a 7% increase, with an independent five-second increase cooldown. WiFi signals include pending sends, Mac drops, repeated Android queue depth, send deficit, rising frame age, rising RTT, and two consecutive windows of classified Android decoder-throughput drops. USB uses stricter link attribution: capture skips, mismatched FPS windows, and short RTT or frame-age changes do not lower bitrate by themselves; real pending sends and Android decoder pressure remain effective. Lifecycle, stale-identity, malformed-input, transport, and codec-reconfiguration drops do not lower bitrate or block stable recovery. Legacy aggregate Android drops are retained for observation but are not guessed to be congestion. Manual and Benchmark never adapt. See the [Android drop-reason policy](android-drop-reason-policy.md).
