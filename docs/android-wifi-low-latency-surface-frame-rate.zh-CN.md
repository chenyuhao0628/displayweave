...
请求的视频 FPS 会映射到最接近的已支持显示刷新率。Window 与 Surface 使用 Fixed-source Compatibility。API 31+ 显式传入 `CHANGE_FRAME_RATE_ONLY_IF_SEAMLESS`；API 30 使用具有相同 Only-if-seamless 默认行为的两参数 Overload。
